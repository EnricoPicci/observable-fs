...
describe('dirNamesListObs function', () => {
    it('reads the subdirectories of a directory', done => {
        const dirPath = 'observable-fs-test-dir/';
        observable_fs_1.dirNamesListObs(dirPath).subscribe(dirs => {
            chai_1.expect(dirs.length).to.equal(2);
            chai_1.expect(dirs.filter(d => d === 'dir-1').length).to.equal(1);
            chai_1.expect(dirs.filter(d => d === 'dir-1').length).to.equal(1);
        }, err => {
            console.error('ERROR', err);
        }, () => {
...
describe('readLineObs function', () => {
    it('reads each line of a file', done => {
        let lineCounter = 0;
        const filePath = 'observable-fs-test-dir/dir-2/file-2-1.txt';
        observable_fs_1.readLineObs(filePath).subscribe(() => lineCounter++, err => {
            console.error('ERROR', err);
        }, () => {
            if (lineCounter !== 5) {
...
describe('appendFileObs function', () => {
    it('appends 2 lines to a file', done => {
        ...
        observable_fs_2.appendFileObs(logFile, linePlusReturn)
            ...
            observable_fs_1.readLinesObs(logFile).subscribe(lines => {
